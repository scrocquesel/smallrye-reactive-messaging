...
public class PulsarClientBaseTest {
    ...
    public static <T> Multi<MessageId> send(Producer<T> producer, Function<Integer, T> generator) {
        return Multi.createFrom().range(0, Integer.MAX_VALUE)
                ...
                .onItem().transformToUniAndConcatenate(value -> Uni.createFrom().emitter(e -> {
                    try {
    ...
    public static <T> Multi<MessageId> send(Producer<T> producer,
        ...
        return Multi.createFrom().range(0, Integer.MAX_VALUE)
                ...
                .onItem().transformToUniAndConcatenate(message -> Uni.createFrom().emitter(e -> {
                    try {
                        e.complete(message.send());
                    } catch (PulsarClientException ex) {
                        e.fail(ex);
                    }
                }));
    ...
    public static <T> List<MessageId> send(Producer<T> producer, long numberOfMessages, Function<Integer, T> generator) {
        return send(producer, generator).select().first(numberOfMessages)
                .subscribe().asStream().collect(Collectors.toList());
    }

    public static <T> List<MessageId> send(Producer<T> producer, long numberOfMessages,
            BiFunction<Integer, Producer<T>, TypedMessageBuilder<T>> generator) {
        return send(producer, generator).select().first(numberOfMessages)
                .subscribe().asStream().collect(Collectors.toList());
    }
